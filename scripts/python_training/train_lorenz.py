#!/usr/bin/env python3
"""
Lorenz Attractor AI Training Script
...
MODEL_DIR = os.path.join(PROJECT_ROOT, 'ai_models', 'lorenz-attractor')
CHECKPOINT_DIR = os.path.join(MODEL_DIR, 'checkpoints')
os.makedirs(MODEL_DIR, exist_ok=True)
os.makedirs(CHECKPOINT_DIR, exist_ok=True)

print("=" * 60)
print("Lorenz Attractor AI Training (LSTM-Transformer Hybrid)")
print("=" * 60)
print(f"TensorFlow version: {tf.__version__}")
try:
    keras_version = keras.__version__
except AttributeError:
    keras_version = "3.x (integrated with TF)"
...
def load_training_data():
    """Load all training data from the lorenz-attractor data directory"""
    ...
    if not data_files:
        ...
        print("Please generate training data first using: node scripts/collect_lorenz_data.js")
        sys.exit(1)
...
def find_latest_checkpoint():
    ...
    checkpoint_files = [f for f in os.listdir(CHECKPOINT_DIR) if f.startswith('lorenz_epoch_') and f.endswith('.keras')]
...
def train_model(model, X_train, y_train, X_val, y_val, epochs=100, initial_epoch=0):
    ...
    callbacks = [
        ...
        keras.callbacks.ModelCheckpoint(
            filepath=os.path.join(CHECKPOINT_DIR, f'lorenz_epoch_{{epoch:04d}}_{timestamp}.keras'),
            save_freq='epoch',
            verbose=1,
        ...
        keras.callbacks.ModelCheckpoint(
            filepath=os.path.join(MODEL_DIR, f'lorenz_best_{timestamp}.keras'),
...
def save_final_model(model, history, metadata, r2_score):
    ...
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    model_name = f'lorenz_final_{timestamp}'
...
def main():
    ...
    try:
        ...
        # Prepare sequences
        X, y = prepare_sequences(datasets, sequence_length=50, prediction_horizon=10)

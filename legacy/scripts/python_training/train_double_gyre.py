#!/usr/bin/env python3
"""
Double Gyre AI Model Training Script
Trains a Hybrid LSTM-Transformer model to predict particle behavior in double gyre flow
Target: 100 epochs with ~30s per epoch, ~1.5M trainable parameters
"""
...
MODEL_DIR = os.path.join(PROJECT_ROOT, 'ai_models', 'double-gyre')
CHECKPOINT_DIR = os.path.join(MODEL_DIR, 'checkpoints')
os.makedirs(MODEL_DIR, exist_ok=True)
os.makedirs(CHECKPOINT_DIR, exist_ok=True)

print("=" * 60)
print("Double Gyre AI Training (LSTM-Transformer Hybrid)")
print("=" * 60)
...
def load_training_data():
    """Load all training data from the double-gyre data directory"""
    ...
    if not data_files:
        ...
        print("Please generate training data first using: python scripts/python_training/collect_double_gyre_data.py")
        sys.exit(1)
...
def prepare_sequences(datasets, sequence_length=50, prediction_horizon=10):
    """
    ...
    for dataset in datasets:
        ...
        for traj_id, features_list in trajectory_groups.items():
            ...
            for i in range(num_timesteps - sequence_length - prediction_horizon):
                ...
                # Target is the state components (x, y) only
                y_seq_state = y_seq[:, :2]  # Extract first 2 features (x, y)
    ...
    # Normalize data to prevent NaN values
    ...
    y_flat = y_scaler.fit_transform(y_flat)
...
def build_hybrid_model(input_shape, output_shape):
    ...
    Smaller, efficient architecture with ~1.5M parameters
    """
    print("\nBuilding Hybrid LSTM-Transformer model...")
    
    inputs = keras.Input(shape=input_shape)
    
    # LSTM layers for temporal feature extraction (reduced size)
    x = layers.LSTM(96, return_sequences=True)(inputs)
    x = layers.Dropout(0.2)(x)
    x = layers.LSTM(96, return_sequences=True)(x)
    x = layers.Dropout(0.2)(x)
    
    # Single Transformer block for attention-based feature refinement
    x = TransformerBlock(embed_dim=96, num_heads=4, ff_dim=256, dropout_rate=0.1)(x)
    
    # Global average pooling to reduce sequence dimension
    x = layers.GlobalAveragePooling1D()(x)
    
    # Dense layers for prediction (reduced)
...
def find_latest_checkpoint():
    ...
    checkpoint_files = [f for f in os.listdir(CHECKPOINT_DIR) if f.startswith('double_gyre_epoch_') and f.endswith('.keras')]
...
def train_model(model, X_train, y_train, X_val, y_val, epochs=100, initial_epoch=0):
    ...
    print(f"Expected time per epoch: ~30s")
    ...
    callbacks = [
        ...
        keras.callbacks.ModelCheckpoint(
            filepath=os.path.join(CHECKPOINT_DIR, f'double_gyre_epoch_{{epoch:04d}}_{timestamp}.keras'),
            save_freq='epoch',
            verbose=1,
        ...
        keras.callbacks.ModelCheckpoint(
            filepath=os.path.join(MODEL_DIR, f'double_gyre_best_{timestamp}.keras'),
...
def save_final_model(model, history, metadata, r2_score):
    ...
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    model_name = f'double_gyre_final_{timestamp}'
...
def main():
    ...
    try:
        ...
        # Prepare sequences
        X, y, X_scaler, y_scaler = prepare_sequences(datasets, sequence_length=50, prediction_horizon=10)

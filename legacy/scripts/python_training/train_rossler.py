#!/usr/bin/env python3
"""
Rössler Attractor AI Training Script
Trains a Hybrid LSTM-Transformer model to predict chaotic dynamics
Target: R² ≥ 0.95
"""
...
MODEL_DIR = os.path.join(PROJECT_ROOT, 'ai_models', 'rossler-attractor')
CHECKPOINT_DIR = os.path.join(MODEL_DIR, 'checkpoints')
os.makedirs(MODEL_DIR, exist_ok=True)
os.makedirs(CHECKPOINT_DIR, exist_ok=True)

print("=" * 60)
print("Rössler Attractor AI Training (LSTM-Transformer Hybrid)")
print("=" * 60)
...
def load_training_data():
    """Load all training data from the rossler-attractor data directory"""
    ...
    if not data_files:
        ...
        print("Please generate training data first using: node scripts/collect_rossler_data.js")
        sys.exit(1)
    ...
    for dataset in datasets:
        ...
        for traj_id, features_list in trajectory_groups.items():
            ...
            for i in range(num_timesteps - sequence_length - prediction_horizon):
                ...
                # Target is the state components (x, y, z) only
                y_seq_state = y_seq[:, :3]  # Extract first 3 features (x, y, z)
...
def find_latest_checkpoint():
    ...
    checkpoint_files = [f for f in os.listdir(CHECKPOINT_DIR) if f.startswith('rossler_epoch_') and f.endswith('.keras')]
...
def train_model(model, X_train, y_train, X_val, y_val, epochs=100, initial_epoch=0):
    ...
    callbacks = [
        ...
        keras.callbacks.ModelCheckpoint(
            filepath=os.path.join(CHECKPOINT_DIR, f'rossler_epoch_{{epoch:04d}}_{timestamp}.keras'),
            save_freq='epoch',
            verbose=1,
        ...
        keras.callbacks.ModelCheckpoint(
            filepath=os.path.join(MODEL_DIR, f'rossler_best_{timestamp}.keras'),
...
def save_final_model(model, history, metadata, r2_score):
    ...
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    model_name = f'rossler_final_{timestamp}'
